...
uses
  ...
  ExtCtrls, SdpoSerial, Registry,dos;
  ...
  OutPutList := TStringList.Create;
   try
     ...
     if  OutPutList.count = 4 then begin
       ...
       //in1
       if trim(a0) <> '' then if (self.CheckBox1.checked) and (self.cb_in1.Text='A0') then (parser1.form1.Panel1.Controls[0] as TCheckBox).Checked:=(StrtoFloat(a0)>600);
       if trim(a3) <> '' then if (self.CheckBox1.checked) and (self.cb_in2.Text='A3') then (parser1.form1.Panel1.Controls[1] as TCheckBox).Checked:=(StrtoFloat(a3)>600);




       parser1.Form1.ButtonClick(parser1.Form1);
   end;
     OutPutList.Free;
   end;
end;
...
begin
  ...
  if not fileexists('config0.cfg') then exit;
  if fileexists(GetEnv('USERNAME')+'_'+GetEnv('USERDOMAIN')+'_config0.cfg') then  memo1.Lines.LoadFromFile(GetEnv('USERNAME')+'_'+GetEnv('USERDOMAIN')+'_config0.cfg')
  else memo1.Lines.LoadFromFile('config0.cfg');
...
begin
  ...
  memo1.lines.SaveToFile( GetEnv('USERNAME')+'_'+GetEnv('USERDOMAIN')+'_config0.cfg');
end;
