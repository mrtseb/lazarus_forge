...
uses
  ...
  ExtCtrls, SdpoSerial, Registry, unit1,utils_serial,dos;
...
begin
  ...
  if fileexists(GetEnv('USERNAME')+'_'+GetEnv('USERDOMAIN')+'_config.cfg') then  memo1.Lines.LoadFromFile(GetEnv('USERNAME')+'_'+GetEnv('USERDOMAIN')+'_config.cfg')
  else memo1.Lines.LoadFromFile('config.cfg');
...
begin
  ...
  memo1.lines.SaveToFile(GetEnv('USERNAME')+'_'+GetEnv('USERDOMAIN')+'_config.cfg');
...
begin
  ...
  self.serial.Device:=self.cb_com.text;
  self.serial.Active:=self.CheckBox1.Checked;
end;
