...
uses
  ...
  ComCtrls, ExtCtrls, singleBit, Types,dos;
...
begin
  if fileexists(GetEnv('USERNAME')+'_'+GetEnv('USERDOMAIN')+'_config2.cfg') then assignfile(f,GetEnv('USERNAME')+'_'+GetEnv('USERDOMAIN')+'_config2.cfg') else assignfile(f,'config2.cfg');
  reset(f);
...
begin
  form2.IdleTimer1.Enabled:=false;
  form2.serial.Close;
