...
uses
  ...
  ExtCtrls, Menus, windows, lazUTF8, SynEdit, SynHighlighterCpp,jclsysutils, dos;
...
type
  ...
  TForm1 = class(TForm)
    ...
    SynEdit1: TSynEdit;
    procedure cb_comChange(Sender: TObject);
...
begin
 ...
 if fileexists(getCurrentDir()+'/ino/'+GetEnv('USERNAME')+'_'+GetEnv('USERDOMAIN')+'_default.ino') then
    self.SynEdit1.Lines.LoadFromFile(getCurrentDir()+'/ino/'+GetEnv('USERNAME')+'_'+GetEnv('USERDOMAIN')+'_default.ino')
 else
    self.SynEdit1.Lines.LoadFromFile(getCurrentDir()+'/ino/default.ino')
end;

procedure TForm1.cb_comChange(Sender: TObject);
begin
  FPort:=self.cb_com.Text;
end;
...
begin
 self.SynEdit1.Lines.SaveToFile(getCurrentDir()+'/ino/'+GetEnv('USERNAME')+'_'+GetEnv('USERDOMAIN')+'_default.ino');
