...
uses
  ...
  Dialogs, StdCtrls, sim_utils, ExtCtrls, tabassoc,
  Menus, ImgList, Buttons, schema_utils, ShellAPI, crt, jclsysutils;
    ...
    try
      ...
      for i:=0 to memo4.lines.count-1 do begin
        ...
        if pos (#9,s) > 0 then begin
         if pos ('alues',s) > 0 then deb:=0;
         split(#9,s,t);

         if t.count=3 then begin
             cles.Add(uppercase(t[t.count-2]));
             memo5.lines.add(uppercase(t[t.count-2]));
         end;
         if t.count <3 then
           begin
              ...
              s:=tt[0]+','+tt[1];
              dico.ajoutElement(cles[deb],s);
              memo5.lines.add(cles[deb]+' '+s);
              deb:=deb+1;
           end;
      end;
...
begin
  ...
  begin
    ...
    FileClose(tmpProcessInformation.hProcess); { *Converti depuis CloseHandle* }
    FileClose(tmpProcessInformation.hThread); { *Converti depuis CloseHandle* }
  end
  else
  begin
    RaiseLastOSError;
...
timer1.Enabled:=true;
//self.caption:= dir+'\circuits\temp.cir';
memo3.lines.SaveToFile(dir+'\circuits\temp.cir');
if fileexists(dir+'\circuits\temp.raw') then deletefile(dir+'\circuits\temp.raw');
secondes:=0;
cmd:='cmd /c '+getcurrentDir()+ '\ngspice\bin\ngspice.exe -b -r '+getcurrentDir()+'\circuits\temp.raw '+getcurrentDir()+'\circuits\temp.cir';
execute(cmd,nil,true,nil);
s:= dir+'\circuits\temp.raw';
while not fileexists(s) do;
memo4.lines.loadFromfile(dir+'\circuits\temp.raw');
self.raw_readtxt;
timer1.Enabled:=false;
analyse;
spicing:=false;
